...
void Lvgl_Touchpad_Read( lv_indev_drv_t * indev_drv, lv_indev_data_t * data )
{
  ...
  if (touchpad_pressed && touchpad_cnt > 0) {
    // Touch IC coordinates are fixed to the panel's native (portrait) sensor
    // layout regardless of the LCD's MADCTL rotation, so swap axes to land
    // in the landscape LVGL screen. This app only needs "was it tapped
    // anywhere" (goToIndex() ignores position), so the exact swap direction
    // doesn't need to match the display rotation precisely - it only needs
    // to keep the whole panel within the LVGL screen bounds.
